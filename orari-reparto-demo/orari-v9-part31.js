...
hoursHtml=function(ds){
  ...
  return`<div class="card"><div class="row wrap"><h3>Ore per addetto</h3><b class="${ot?'bad':'ok'}">Straordinari ${hf(ot)}</b></div><div class="scroll"><table><thead><tr><th>Addetto</th><th>Contr.</th><th>Target</th><th>Lavor.</th><th>Perm.</th><th>Corso/libero</th><th>Mal.</th><th>Spost.</th><th>Fest.</th><th>Manc.</th><th>Extra</th><th></th></tr></thead><tbody>${p.map(e=>`<tr class="${e.cr?'crrow':''}"><td><b>${esc(e.name)}</b><br><small>${e.cr?'CR':e.dept==='carni'?'Carni':'Gastronomia'}</small></td><td>${hf(e.hours)}</td><td><b>${hf(e.workTarget)}</b></td><td>${hf(e.worked)}</td><td>${e.permits?hf(e.permits):'—'}</td><td>${e.dayStatusCount?hf(e.dayCredits):'—'}</td><td>${e.sickDays?`${e.sickDays} gg`:'—'}</td><td>${e.moved?hf(e.moved):'—'}</td><td>${e.festive?hf(e.festive):'—'}</td><td>${e.missing?hf(e.missing):'—'}</td><td class="${e.extra?'bad':''}">${e.extra?hf(e.extra):'—'}</td><td><div class="row wrap" style="gap:5px">${typeof singleDayButton==='function'?singleDayButton(S.employees.findIndex(x=>x.name===e.name)):''}<button class="btn small" onclick="quickLeave('${esc(e.name)}')">Ferie</button><button class="btn small" onclick="quickSickness('${esc(e.name)}')">Malattia</button></div></td></tr>`).join('')}</tbody></table></div><small class="muted">Target = ore ordinarie da distribuire dopo ferie/permessi, malattia, spostamenti e festivita. Corso/libero: ore riconosciute nel totale personale, escluse dalle ore lavorate nei reparti.</small></div>`;
};
